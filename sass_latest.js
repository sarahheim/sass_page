...
*/
  ...
  $.each(attrsArr , function(a, attr) {
    valTxt = staDic[attr]['value'] +' '+ staDic[attr]['unit']
    //ATTRIBUTE HAS FLAG
...
/*
    var dataSpan = document.createElement("span");
    dataSpan.setAttribute('id', 'latestSpan_'+sta+'_'+attr);
//    dataSpan.innerHTML = attr+': <button type="button" id="latest_'+sta+'_'+attr+'" class="'+className+'">'+valTxt+'</button><br>';
    dataSpan.innerHTML = attr+': <button type="button" id="latest_'+sta+'_'+attr+'" class="btn btn-compact btn-link">'+valTxt+'</button>'+icon+'<br>';
    if (flagsArr.indexOf(attr) == -1) dataSpan.setAttribute('style','display:none'); 
    $('#'+dataId).append(dataSpan);
*/
